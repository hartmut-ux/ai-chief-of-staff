---
name: chief-of-staff
description: Codex skill wrapper for the AI-agnostic chief_of_staff engine. Run a personal AI chief of staff that gathers email, calendar, tasks, web, and custom sources in parallel, synthesizes one prioritized daily briefing, and delivers it to your chosen channel.
---

# Chief of Staff — Codex wrapper

This is the **Codex skill wrapper** for the `chief_of_staff` Python engine. The skill is intentionally thin: all heavy logic lives in the top-level `chief_of_staff/` package. This wrapper only describes how to invoke the engine from Codex.

Use this skill when the user asks for:
- A daily briefing, morning rundown, or “what do I need to know today?”
- A Codex prompt like “run my chief of staff briefing”
...
Option A — spawn parallel Codex subagents, one per enabled source. Each subagent:

1. Connects via its MCP server, API, or Codex built-in tool.
...
| Web       | Codex web search or a custom scraping script         | `memory/source-cache/web.json`      |
